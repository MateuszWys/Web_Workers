...
<details>
  ...
  <div>
      <label for="workers-count">Choose number of webworkers:</label>
      <input id="workers-count" name="workers-count" type="number" value="10" />
  </div>  
  <div>
      <label for="columns-count">Choose number of columns:</label>
      <input id="columns-count" name="columns-count" type="number" value="10" />
  </div>
  ...
  startPreprocessing = () => {
    ...
    for (const ranges of columnsRanges) {
      for (const [start, end] of ranges) {
        ...
        const worker = new Worker('./workers/histogramColorWorker.js');
